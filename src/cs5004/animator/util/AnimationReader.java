...
public class AnimationReader {
  /**
   * A factory for producing new animations, given a source of Shape and a builder for constructing
   * animations.
   *
   * <p>
   ...
   * @param <Doc>    The main cs5004.animator.model interface type describing animations
   * @return
   */
  public static <Doc> Doc parseFile(Readable readable, AnimationBuilder<Doc> builder) {
    ...
    Objects.requireNonNull(builder,
            "Must provide a non-null cs5004.animator.util.AnimationBuilder");
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    String[] fieldNames = new String[]{"initial time", "initial x-coordinate", "initial "
            + "y-coordinate", "initial width", "initial height", "initial red value", "initial "
            + "green value", "initial blue value", "final time", "final x-coordinate", "final "
            + "y-coordinate", "final width", "final height", "final red value", "final green value",
        "final blue value"};
    int[] vals = new int[16];
    String name;
    if (s.hasNext()) {
